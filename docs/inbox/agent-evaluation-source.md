# 原始资料:Agent评测漫谈 —— 由浅入深讲解Agent评测

> 来源:微信公众号「图灵Agent评测」(美团图灵团队),原文链接 https://mp.weixin.qq.com/s/gZKWRqznB8sNBFf69fBIvw
> 抓取日期:2026-08-09;状态:已整理为正式文章 docs/03-agents/agent-evaluation.md
...
task定义
pinchbench
2026年2月开源，是一套专门用于评估OpenClaw的性能基准测试系统。与传统的合成测试（Synthetic Tests）不同，PinchBench 强调“真实场景下的任务模拟”。它通过给 AI 代理布置实际工作中会遇到的复杂任务，来衡量模型在处理多步工作、实际开发与办公环境下的真实表现。
1200+
md文件
claw eval
北大发布的龙虾能力评测task列表  
...
yaml文件
WildClawBench
WildClawBench：野生环境 AI Agent 能力评测 10 大模型谁的"龙虾"最强？核心理念是"在野生环境中测试 Agent"——不是给模型一个精心设计的沙盒，而是把它扔进真实用户每天使用 OpenClaw Agent 的场景中，看它能不能活下来。
500
Skill
