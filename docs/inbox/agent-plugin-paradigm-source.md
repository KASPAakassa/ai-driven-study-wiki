> 原文存档:微信文章《一切皆插件之后,Agent 工程的新范式》(公众号:阿里云云原生)
> 原始链接:https://mp.weixin.qq.com/s/OgYIw9l8PBeRk5MGMhYTmA
> 抓取日期:2026-08-14(手机 UA curl,避开微信环境验证)
> 用途:整理收件箱素材(用户要求:新增"基于插件的 Agent 开发范式"文章 + 扩充思考),正文原样保留供追溯。
...
开发者主要和 Agent、Chain、Tool 这些对象打交道，真正执行时发生了什么，例如上下文如何被拼接、工具结果如何被截断或注入、循环如何决定继续或停止，**往往需要额外开启 tracing、翻源码或依赖阿里云****[https://mp.weixin.qq.com/s?__biz=MzUzNzYxNjAzMg==&mid=2247584866&idx=1&sn=27cebff1f04e7b53bab7805d4339fd75&scene=21#wechat_redirect](https://mp.weixin.qq.com/s?__biz=MzUzNzYxNjAzMg==&mid=2247584866&idx=1&sn=27cebff1f04e7b53bab7805d4339fd75&scene=21#wechat_redirect)**** 这类外部观测工具才能看清楚。**一旦出问题，例如幻觉、死循环、工具调用混乱、成本暴涨，定位经常要穿过多层抽象，才能碰到真实的数据流和事件流。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/bvDbzNRia8j0FEjmJiba1py4Ft6e1oVuJDJVbCibP5ic3XxgJXAgfIxQCFI6icHkFGxJ4A8icoK9ibYpVQwbGg8pZJicibj5c13Ar7yK1y964vApfC4E/640?wx_fmt=png&from=appmsg)

****运行中的 dsh 就是这棵树：启动时按固定顺序把各层配置叠在一起，最终形成实际生效的插件集合。
...
dsh-web-app：在 base 之上增加浏览器 Web UI。
...
自己保存一份 cordis.patch.yml（用户针对这个 Profile 的个性化补丁）。
官方提供 web 和 headless 两个模板 Profile。
...
创建 cordis.patch.yml，用 insert 或按 id 替换，然后：
...
插件通过 ctx 注册服务（如 ctx.tools、ctx.llm、ctx.sessions）。其他插件用 inject 声明依赖，运行时自动获得对应能力。
...
正式贡献：参考仓库的 CONTRIBUTING.md、docs/cookbook/（有添加 package、tool、LLM adapter、Chat node 的指南），以及架构文档。
-
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/bvDbzNRia8j1pO4icRqPJ6YUhKNIxNIm6XFlVpLGL2PyiabN7ocX7U5eCsd7MBkWsyGPVWO3ZDEEianpbgKpKwiacwqKC3P4b5pyVzyvGcibG7FKM/640?wx_fmt=png&from=appmsg)
